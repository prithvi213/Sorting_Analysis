#include "quick.h"

#include <math.h>
#include <stdio.h>
#include <stdlib.h>

// Used pseudocode from Assignment 3
// Inspired from Professor Long
// Partition function breaks down arrays and sorts them
uint32_t partition(Stats *stats, uint32_t *A, uint32_t lo, uint32_t hi) {
    uint32_t i = lo - 1;

    for (uint32_t j = lo; j < hi; j++) {
        if (cmp(stats, A[j - 1], A[hi - 1]) == -1) {
            i++;
            swap(stats, &A[i - 1], &A[j - 1]);
        }
    }

    swap(stats, &A[i], &A[hi - 1]);
    return (i + 1);
}

// Quicksort algorithm
// Inspired from Professor Long
// Used pseudocode from Assignment 3
void quick_sorter(Stats *stats, uint32_t *A, uint32_t lo, uint32_t hi) {
    if (lo < hi) {
        // Partitions the array
        // Performs quick sorts on both partitions
        uint32_t p = partition(stats, A, lo, hi);
        quick_sorter(stats, A, lo, p - 1);
        quick_sorter(stats, A, p + 1, hi);
    }
}

// Calls the quick_sorter function
void quick_sort(Stats *stats, uint32_t *A, uint32_t n) {
    quick_sorter(stats, A, 1, n);
}
